...
parser.add_argument(
    ...
    "--gamma",
    dest="gamma",
    help="Parameter for exponential average in momentum",
    default=0.9,
    type=float,
)
...
# mlp = MLP(input_size=num_features, output_size=num_classes, hidden_mult=8)
mlp = MLP(input_size=num_features, output_size=num_classes, hidden_mult=2)
...
for epoch in range(args.epochs):
    ...
    momentum = [np.zeros(p.size()) for p in mlp.parameters()]
    for fr in range(0, n, b):
        ...
        for id, parm in enumerate(mlp.parameters()):
            momentum[id] = args.gamma * momentum[id] + parm.grad
            parm.value -= args.lr * momentum[id]
